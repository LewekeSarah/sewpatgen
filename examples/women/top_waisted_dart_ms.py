...
def make_measurements(
    person: Person, ease_TaU: float = 6.0 * CM, ease_HüU: float = 4.0 * CM
) -> TopMeasurements:
...
def _bezier_neck_back(
    corner: Point, shoulder_end: Point, spine_top: Point
) -> CubicBezier:
...
def _bezier_neck_front(
    ...
    return CubicBezier(
        neckline_shoulder, cp1, cp2, neckline_centre, name="vordere Halsrundung"
    )
...
def make_top(meas: TopMeasurements, model_length: float = 55.0 * CM) -> Pattern:
    ...
    grid = ConstructionGrid(
        ...
        verticals=[
            ...
            (
                "vordere Armlinie",
                pt7_shift + meas.RüB + meas.ArD * 2 / 3 + 10 * CM + meas.ArD / 3,
            ),
    ...
    # back armscye shape.
    curve_arm_b = _bezier_armscye_back(pt_shoulder_back, pt_bust_srt, meas.ArD)
    ...
    seg_side_bust_waist_b = Segment(
        pt_side_bust_b, pt_side_waist_b, "Seitennaht Rücken ob."
    )
    seg_side_waist_hip_b = Segment(
        pt_side_waist_b, pt_side_hip_b, "Seitennaht Rücken u."
    )
    ...
    dart_b_right = Point(
        dart_b_centre_x + half_dw_b, pt_waist_ha.y, "Abnäher hR rechts"
    )
    ...
    back.add_info_box(
        header="Rücken",
        notes=[
            f"Nahtzugabe {meas.sa / CM:.0f} cm (außer Stoffbruch)",
            "1× zuschneiden, auf Falte legen",
        ],
    )
    ...
    pt_neck_cf_bottom = pt_shoulder_vm.translate(
        0, neck_depth_f
    )  # centre-front neck depth
    ...
    curve_arm_f = _bezier_armscye_front(pt_bust_svt, pt_shoulder_front, meas.ArD)
    front.append(curve_arm_f, is_outline=True)

    # ── Side seam — front ──────────────────────────────────────────────────
    side_reduction_front = (
        total_waist_reduction * 0.15
    )  # front side-seam share (= back)
    ...
    seg_side_bust_waist_f = Segment(
        pt_side_bust_f, pt_side_waist_f, "Seitennaht Vorderteil ob."
    )
    seg_side_waist_hip_f = Segment(
        pt_side_waist_f, pt_side_hip_f, "Seitennaht Vorderteil u."
    )
    seg_side_hip_hem_f = Segment(
        pt_side_hip_f, pt_side_hem_f, "Seitennaht Vorderteil Saum"
    )
    ...
    seg_cf_bust_neck = Segment(
        pt_bust_vm, pt_neck_cf_bottom, "vordere Mitte Brust–Hals"
    )
    ...
    dart_f_left = Point(
        dart_f_centre_x - half_dw_f, pt_waist_va.y, "Taillenabnh. vorn links"
    )
    dart_f_right = Point(
        dart_f_centre_x + half_dw_f, pt_waist_va.y, "Taillenabnh. vorn rechts"
    )
