...
# ---------------------------------------------------------------------------
# Helper for PatternElement.split_at_dart tests
# ---------------------------------------------------------------------------


def _make_dart(seg: Segment) -> Dart:
...
def test_split_at_dart_segment_splits_into_two_outer_parts():
...
def test_split_at_dart_children_inherit_style():
    ...
    for child in children:
        assert child.style == STYLE_STITCH


def test_split_at_dart_children_inherit_is_outline():
    ...
    children = elem.split_at_dart(dart)
    for child in children:
        assert child.is_outline


def test_split_at_dart_non_segment_raises_type_error():
...
def test_split_at_dart_cubic_bezier_splits():
...
def test_precision_point_default_radii():
    """Default outer and inner radii are correct."""
    p = Point(5 * CM, 5 * CM)
    pp = PrecisionPoint(p)
    assert pp.outer_radius == pytest.approx(5.0 * MM)
    assert pp.inner_radius == pytest.approx(0.5 * MM)


def test_precision_point_custom_radii():
    """Custom radii are stored correctly."""
    p = Point(0, 0)
    pp = PrecisionPoint(p, outer_radius=3 * MM, inner_radius=0.5 * MM)
    assert pp.outer_radius == pytest.approx(3 * MM)
    assert pp.inner_radius == pytest.approx(0.5 * MM)


def test_precision_point_build_elements_returns_two_circles():
...
def test_precision_point_build_elements_are_circles():
    """The two elements wrap Circle geometry."""
    p = Point(0, 0)
    pp = PrecisionPoint(p)
    elems = pp.build_elements()
    for e in elems:
        assert isinstance(e.geometry, Circle)


def test_precision_point_custom_style_applied():
