"""Tests for Block and OverlayPart subclasses, including explode().

Covers:
  - Block: subclass identity, render exclusion, geometry reuse
  - OverlayPart: parent, no anchor, explode (geometry, flags, deep copy, attributes)
"""
...
from .conftest import _front_part, _pocket_overlay
...
def test_overlay_is_instance_of_pattern_part():
    assert isinstance(OverlayPart(name="Tasche", parent=_front_part()), PatternPart)


def test_overlay_is_instance_of_overlay_part():
    assert isinstance(OverlayPart(name="Tasche", parent=_front_part()), OverlayPart)


def test_overlay_parent_stored():
    front = _front_part()
    assert OverlayPart(name="Tasche", parent=front).parent is front


def test_overlay_no_anchor_attribute():
    assert not hasattr(OverlayPart(name="Tasche", parent=_front_part()), "anchor")


def test_overlay_name_stored():
    assert OverlayPart(name="Tasche", parent=_front_part()).name == "Tasche"


def test_overlay_accepts_initial_elements():
    front = _front_part()
...
def test_explode_element_count_preserved():
    _, pocket = _pocket_overlay()
    assert len(pocket.explode(offset=Point(110, 0)).elements) == len(pocket.elements)


def test_explode_geometry_translated():
...
def test_explode_is_outline_flag_preserved():
    _, pocket = _pocket_overlay()
    result = pocket.explode(offset=Point(110, 0))
    for orig, expl in zip(pocket.elements, result.elements, strict=False):
...
def test_explode_is_seam_allowance_flag_preserved():
    _, pocket = _pocket_overlay()
...
def test_explode_style_preserved():
    front = _front_part()
...
def test_explode_does_not_mutate_original():
    _, pocket = _pocket_overlay()
...
def test_explode_does_not_mutate_parent():
    front, pocket = _pocket_overlay()
    count_before = len(front.elements)
    pocket.explode(offset=Point(110, 0))
    assert len(front.elements) == count_before


def test_overlay_excluded_from_default_export():
...
def test_explode_role_preserved():
    front = _front_part()
...
def test_explode_is_construction_preserved():
    front = _front_part()
    pocket = OverlayPart(name="Pocket", parent=front)
...
def test_explode_is_seam_notch_preserved():
    front = _front_part()
    pocket = OverlayPart(name="Pocket", parent=front)
...
def test_explode_element_name_preserved():
    front = _front_part()
    pocket = OverlayPart(name="Pocket", parent=front)
...
def test_explode_style_deep_copied():
...
def test_explode_sa_center_translated():
    front = _front_part()
    pocket = OverlayPart(name="Pocket", parent=front)
    elem = pocket.append(Segment(Point(10, 10), Point(40, 10)), is_outline=True)
...
def test_explode_dart_ref_cleared():
...
def test_explode_leg_pt_translated():
    front = _front_part()
    pocket = OverlayPart(name="Pocket", parent=front)
    elem = pocket.append(Segment(Point(10, 10), Point(40, 10)))
