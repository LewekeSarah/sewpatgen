...
def _make_nameless_factory() -> DartElements:
    edge = PatternElement(Segment(Point(0, 0), Point(100, 0)), style=StyleOptions())
    return DartElements.from_edge(edge, position_t=0.5, width=20 * MM, depth=50 * MM)
    # name is intentionally omitted → None


def test_add_dart_no_name_tip_elements():
    part = PatternPart("test")
    result = part.add_dart(_make_nameless_factory(), notches=False, precision_tip=True)
    # Only two circles — no InfoBox when name is None
    assert len(result.tip_elements) == 2


def test_svg_export_no_name(tmp_path):
    part = PatternPart("test")
    part.add_dart(_make_nameless_factory(), notches=False, precision_tip=True)
...
def test_svg_export_direct_dart_no_name(tmp_path):
    """Dart constructed directly (not via from_edge) without a name."""
    part = PatternPart("test")
    dart = Dart(
